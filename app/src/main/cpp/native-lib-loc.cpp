...
extern "C"
jstring JNICALL
Java_com_example_mistepcount_NativeLibraryLoc_stringFromJNI(JNIEnv *env, jobject instance) {
...
extern "C" jint
Java_com_example_mistepcount_NativeLibraryLoc_passingDataToJni(JNIEnv *env,
                                                               jobject instance,
    ...
    double stdDevthreshold=0;  // Change this value to adjust the sensitivity of the heuristic
...
//     Calculate the standard deviation of the input data
    ...
    if (stdDev > stdDevthreshold) {
        ...
        filtered = lowPassFilter(input, cutoffFrequency, sampleRate, 55);
    } else {
